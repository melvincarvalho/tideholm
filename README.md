...
| `BOT_MORALE_FLOOR` | = `MORALE_FLOOR` | Same, but vs **bot** defenders. Season 6 ran at `1` (the leader fights bots at full power) until 2026-09-08, then `0.3` like humans: a big player farming small bots pays the same morale tax as bullying a small human. |
| `WONDER_WIN_COUNT` | 1 | How many winning-level Beacons ONE captain must hold at once. At `2`, build both (~2× the wonder bill — the pacifist premium) or build one and capture a rival's: conquest carries buildings intact, so an enemy's rising tower is a prize as much as a threat, and stealing a lone finished beacon no longer wins by itself. Wonder stays personal — alliances have the crown. Clamped `[1, 5]`. See #178. |
| `CHAIN_API` | `https://mempool.guide/testnet4/api` | The esplora API the provably-fair verifier reads block hashes from — txbt4, XBT's testnet4 (the BLAKE2b fork), by default. Plain testnet4 is `https://mempool.space/testnet4/api`, a different chain above block 150,307. |
| `CHAIN_PIN` | `150400:00000000c4ad…6df6` | `<height>:<hash>` the verifier checks on its first read, so a wrong `CHAIN_API` refuses loudly instead of settling dice against another chain. `off` skips it. |
| `TIDEGATE_NETWORK` | txbt4 | The chain the seal writes on: named in every anchor stamp (explorer links follow it), reported to the client as `state.chain` (anchors go there whatever the Market's chain dropdown views). `tbtc4` for plain testnet4. |
| `WIN_BASIS` | all | What `WIN_SHARE` is a share of: `all` islands on the map, or `populated` (inhabited only). Under season-6 curves a crown priced against the whole map is a decoy — the flagship ladder to 23 islands costs a second beacon — while half the *living* world tracks beacon-cost all season and rises as colonies land. See #177. |
| `POOL_TRADE_SPEED` / `POOL_TRAVEL_MIN` / `POOL_TRAVEL_MAX` | 8 / 30 / off | The Tidepool's sails (#176): minutes per field for pool deliveries, the floor, and an optional ceiling in minutes (`0` = uncapped). Defaults reproduce #76's behaviour exactly (pool boats match `TRADE_SPEED`, 30-min floor, no cap). Distance stays real — the #30 brake holds — but the market's boats can be tuned apart from merchant convoys. Loosening mid-season strands nothing, so these are env knobs, not world stamps. Season 6 runs `2 / 10 / 100`. |
| `TRANSIT_POP_FACTOR` | 0 (free) | What fraction of its population an army costs while at sea — troops aboard attack, return, scout and colonize voyages charge `pop × factor` (ceil) against their home farm. `0` is the old behaviour: transit was population-free, so perpetual raiding pipelined a standing force unboundedly past the cap (train into the gap while the army sails; nothing re-checks on return). `0.5` — half rations at sea — keeps campaign surges possible while bounding the pipeline at ~2× cap; `1` is strict. Support is #40's domain at full cost under its own world flag. Clamped to `[0, 1]`, junk falls back to free. See #174. |
| `FLAGSHIP_COST_GROWTH` | 1 (flat) | The conquest unit rides the same positional curve as the Colony Ship, under its own knob — position is islands owned plus flagships already paid for (garrison, queue, aboard any movement), so batches cannot dodge. With settling taxed and flagships flat, width-by-conquest bypassed the expansion tax ~10× cheaper at position 10. Tune softer than `COLONY_COST_GROWTH`: conquest already pays in blood (`1.2` puts the 10th flagship at ~29k all-in, the 15th ~73k). Same `[1, 3]` clamp as #61. See #173. |
| `FLAGSHIP_STORAGE_CLAMP` | 1 (on) | A flagship's price is clamped to the launching isle's storehouse, resource by resource, while at least one of the three still fits. Under #173 every resource climbs the same ladder, so wood overflows first and gold four rungs later; those four rungs used to be unbuyable for a non-money reason ("no storehouse can hold it") and now cost a full storehouse of the overflowed resource. Once all three overflow the bare curve returns — unaffordable until the storehouse grows. Net: four more flagships per storehouse level, ~8% off the bill to isle 24, nothing off the calendar. `0` restores the bare curve. See #182. |
| `BOT_RESPAWN` | 1 (on) | Whether a bot wiped to its last island receives a refuge. Set `0` and a wiped bot is eliminated — the season's `BOTS` become a finite population and attrition consolidates the endgame into PvP. Never gates humans. See #172. |
| `BOT_GARRISON_RATIO` | ∞ | Cap a bot's standing defence to `islandPoints × ratio × persona.defenseRatio` (raiders exempt, so a capped bot turns to attacking). Set e.g. `12` to stop bots turtling into unbeatable fortresses. |
| `BOT_GARRISON_SETTLER` · `BOT_GARRISON_WARLORD` · `BOT_GARRISON_BARBARIAN` | `BOT_GARRISON_RATIO` | The same cap per temperament, so one kind can be armed without arming the rest. Season 6 runs settlers and warlords at `18` and barbarians at `12`; a barbarian's persona halves its own (`defenseRatio` 0.5), so their effective cap stays at 6 and the gold wells stay farmable. Unset = the base ratio for everyone, which is what every test and the golden log run. |
