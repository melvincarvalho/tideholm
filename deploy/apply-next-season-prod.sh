...
#   3. Computes WORLD_START = next 18:00 Europe/Berlin, or uses $1 (ISO or ms).
...
# WORLD_START: arg (epoch ms or ISO) or next 18:00 Europe/Berlin.
if [ -n "$ARG_START" ]; then
  if [[ "$ARG_START" =~ ^[0-9]+$ ]]; then START_MS="$ARG_START";
  else START_MS=$(( $(date -d "$ARG_START" +%s) * 1000 )); fi
else
  NOW=$(TZ=Europe/Berlin date +%s)
  TODAY17=$(TZ=Europe/Berlin date -d "18:00" +%s)
  if [ "$NOW" -lt "$TODAY17" ]; then START_S=$TODAY17;
  else START_S=$(TZ=Europe/Berlin date -d "tomorrow 18:00" +%s); fi
...
echo "== verify =="
curl -sf -o /dev/null -w "http: %{http_code}\n" "http://localhost:$PORT/tideholm/" || { echo "SERVER NOT RESPONDING"; exit 1; }
curl -sf "http://localhost:$PORT/tideholm/api/admin/stats?token=$ADMIN_TOKEN" | python3 -c "
import json, sys
